...
nsresult nsHttpTransaction::Init(
  ...
  if (mActivityDistributor) {
    rv = mActivityDistributor->ObserveActivityWithChannelId(
...
void nsHttpTransaction::OnTransportStatus(nsITransport* transport,
  ...
  if (mActivityDistributor) {
    ...
    if ((mHasRequestBody) && (status == NS_NET_STATUS_WAITING_FOR)) {
      nsresult rv = mActivityDistributor->ObserveActivityWithChannelId(
          mChannelId, NS_HTTP_ACTIVITY_TYPE_HTTP_TRANSACTION,
    ...
    nsresult rv = mActivityDistributor->ObserveActivityWithChannelId(
        mChannelId, NS_HTTP_ACTIVITY_TYPE_SOCKET_TRANSPORT,
