...
class UpdateVerifyConfigCreator(BaseScript, VirtualenvMixin):
    config_options = [
        ...
        [["--buildhub-api"], {
            "dest": "buildhub_api",
            "default": "https://buildhub.prod.mozaws.net/v1/buckets/build-hub",
            "help": "",
        }],
        [["--full-check-locale"], {
    ...
    def _get_update_paths(self):
        ...
        from mozrelease.paths import getBuildhubReleaseUrl
        ...
        for release_name, release_info in releases.items():
            ...
            buildhub_url = getBuildhubReleaseUrl(
                self.config["buildhub_api"], self.config["stage_product_name"],
                version, self.config["platform"],
            )
            ret = requests.get(buildhub_url)
            self.log("Retrieving buildid from buildhub: %s" % buildhub_url, level=DEBUG)
            if not ret.ok:
                raise Exception("Couldn't find buildid for %s - cannot continue." % release_name)
            buildID = ret.json()["data"]["build"]["id"]
