...
static bool
GetNumberProperty(JSContext* aCx, HandleObject aObject, const char* aProperty, bool aRequired,
                  Number* aResult)
{
  RootedValue v(aCx);
...
DecodeBreakpointPosition(JSContext* aCx, HandleObject aObject, BreakpointPosition* aResult)
{
  *aResult = BreakpointPosition();
  ...
  return GetNumberProperty(aCx, aObject, "script", false, &aResult->mScript)
      && GetNumberProperty(aCx, aObject, "offset", false, &aResult->mOffset)
      && GetNumberProperty(aCx, aObject, "frameIndex", false, &aResult->mFrameIndex);
}
...
EncodeExecutionPoint(JSContext* aCx, const ExecutionPoint& aPoint)
{
  ...
  if (!obj || !position ||
      ...
      !JS_DefineProperty(aCx, obj, "position", position, JSPROP_ENUMERATE))
  {
    return nullptr;
  }
  return obj;
...
static bool
DecodeExecutionPoint(JSContext* aCx, HandleObject aObject, ExecutionPoint* aPoint)
{
  *aPoint = ExecutionPoint();

  RootedValue v(aCx);
  if (!JS_GetProperty(aCx, aObject, "position", &v)) {
    return false;
  }
...
Middleman_TimeWarp(JSContext* aCx, unsigned aArgc, Value* aVp)
{
  ...
  if (!DecodeExecutionPoint(aCx, targetObject, &target)) {
    return false;
  }
...
RecordReplay_CurrentExecutionPoint(JSContext* aCx, unsigned aArgc, Value* aVp)
{
  ...
  RootedObject result(aCx, EncodeExecutionPoint(aCx, point));
  if (!result) {
    return false;
  }
