...
extern {
    ...
    fn RecordReplayRust_BeginPassThroughThreadEvents();
    fn RecordReplayRust_EndPassThroughThreadEvents();
}
...
pub struct AtomicIsizePreserved {
    data: Mutex<AtomicIsize>,
}

impl AtomicIsizePreserved {
    pub fn new(v: isize) -> AtomicIsizePreserved {
        AtomicIsizePreserved {
            data: Mutex::new(AtomicIsize::new(v)),
        }
    }
    ...
    pub fn store(&self, v: isize, order: Ordering) {
        self.data.lock().unwrap().store(v, order)
    }

    pub fn swap(&self, ptr: isize, order: Ordering) -> isize {
...
struct Inner<T> {
    /// The bottom index.
    bottom: AtomicIsizePreserved,

    /// The top index.
    top: AtomicIsizePreserved,
...
impl<T> Inner<T> {
    ...
    fn with_min_capacity(min_cap: usize) -> Self {
        ...
        Inner {
            bottom: AtomicIsizePreserved::new(0),
            top: AtomicIsizePreserved::new(0),
...
impl<T> Deque<T> {
    ...
    pub fn steal(&self) -> Steal<T> {
        unsafe { RecordReplayRust_Assert("Deque::steal\0".as_ptr()); }
...
impl<T> Stealer<T> {
    ...
    /// [`Steal::Retry`]: enum.Steal.html#variant.Retry
    pub fn steal(&self) -> Steal<T> {
        unsafe { RecordReplayRust_Assert("Stealer::steal #1\0".as_ptr()); }

        // Load the top.
        let t = self.inner.top.load(Acquire);

        unsafe { RecordReplayRust_Assert("Stealer::steal #2\0".as_ptr()); }

        // Workaround recorded events occurring non-deterministically within
	// call-once initialization code triggered below.
        unsafe { RecordReplayRust_BeginPassThroughThreadEvents(); }
        ...
        unsafe { RecordReplayRust_EndPassThroughThreadEvents(); }

        unsafe { RecordReplayRust_Assert("Stealer::steal #3\0".as_ptr()); }

        let guard = &epoch::pin();

        unsafe { RecordReplayRust_Assert("Stealer::steal #4\0".as_ptr()); }

        // Load the bottom.
        let b = self.inner.bottom.load(Acquire);

        unsafe { RecordReplayRust_Assert("Stealer::steal #5\0".as_ptr()); }
