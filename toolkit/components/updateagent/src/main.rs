...
// prevents Windows consoles from picking up output, however, so one might want to disable it for
// debugging.
...
pub mod cmd {
    ...
    pub fn parse(s: &str) -> Option<Command> {
        ...
        if s == REGISTER_TASK {
            Some(RegisterTask)
        } else if s == REGISTER_TASK_LOCAL_SERVICE {
            Some(RegisterTaskLocalService)
        } else if s == UPDATE_TASK {
            Some(UpdateTask)
        } else if s == UPDATE_TASK_LOCAL_SERVICE {
            Some(UpdateTaskLocalService)
        } else if s == UNREGISTER_TASK {
            Some(UnregisterTask)
        } else if s == RUN_ON_DEMAND {
            Some(RunOnDemand)
        } else if s == DO_TASK {
            Some(DoTask)
        } else {
...
pub fn fallible_main() -> Result<(), String> {
    let command = env::args().nth(1).ok_or("missing command")?;
    let command = cmd::parse(&command).ok_or_else(|| format!("unknown command \"{}\"", command))?;

    // Most commands use the second arg as task name
    let args: Vec<_> = env::args_os().collect();
    let maybe_task_name = args.get(2).ok_or("missing task name");
    ...
    match command {
        ...
        | Command::UpdateTaskLocalService => {
            ...
            task_setup::register(&*exe, &maybe_task_name?, &args[3..], command)
                .map_err(|e| format!("register failed: {}", e))
        }
        Command::UnregisterTask => {
            if args.len() != 3 {
                return Err("unregister takes one argument: TaskName".into());
            }
        ...
        Command::RunOnDemand => {
            if args.len() != 3 {
                return Err("run-on-demand takes one argument: TaskName".into());
            }

            task_setup::run_on_demand(&maybe_task_name?)
                .map_err(|e| format!("run on demand failed: {}", e))
        }
        Command::DoTask => task_action(&args[2..]),
    }
}
