...
/**
 * This function wraps observers so that their methods are called with a
 * BitsRequest rather than a nsIBitsRequest.
 *
 * This function may throw if observer is not an nsIRequestObserver.
 *
 * This wrapper will implement nsIProgressEventSink even if the passed observer
 * does not. Ideally it would not do this, but currently nsIBits does not
 * optimize for the case where the observer is not an nsIProgressEventSink,
 * so the impact of doing so here as well is minimal.
 */
function wrapObserver(observer) {
  ...
  return {
    ...
    onProgress: function wrappedObserver_onProgress(request, context, progress,
                                                    progressMax) {
    ...
    onStatus: function wrappedObserver_onStatus(request, context, status,
                                                statusArg) {
      if (isProgressEventSink) {
        observer.onStatus(new BitsRequest(request), context, status, statusArg);
      }
    },
...
var Bits = {
  ...
  async startDownload(downloadURL, saveRelPath, proxy, monitorIntervalMs,
    ...
    return new Promise((resolve, reject) => {
      ...
      let wrappedObserver;
      try {
        wrappedObserver = wrapObserver(observer);
      } catch (e) {
