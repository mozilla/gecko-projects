...
use bits_interface::BitsRequest;

use bits_client::{
    bits_protocol::HResultMessage,
    BitsJobState, BitsMonitorClient, Guid, JobStatus, PipeError
};
...
impl MonitorRunnable {
    ...
    fn free_mainthread_data(&self) {
        if is_main_thread() {
            // This is not safe to free on the main thread
        ...
        if is_main_thread() {
            ...
            let maybe_next_thread: Option<RefPtr<nsIThread>> = {
                ...
                if transfer_completed(&status) {
                    let job_status = request.get_status();
                    let transfer_status = status_to_nsresult(&status);

                    // The status_to_nsresult function is pretty vague. It mostly just gives
                    // NS_OK or NS_ERROR_FAILURE, so if the request already knows why it is
                    // in the state that it is in, just trust that its code is more accurate.
                    let new_status = if job_status.succeeded() != transfer_status.succeeded() {
                        Some(transfer_status)
                    } else {
                        None
                    };

                    request.on_stop(new_status);
