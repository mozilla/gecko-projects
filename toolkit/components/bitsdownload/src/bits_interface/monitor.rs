...
use bits_client::{BitsJobState, BitsMonitorClient, JobStatus, PipeError};
...
use xpcom::{
    interfaces::{
        nsIEventTarget, nsIProgressEventSink, nsIRequestObserver, nsISupports, nsIThread,
    },
    xpcom, xpcom_method, GetterAddrefs, RefPtr, ThreadBoundRefPtr, XpCom,
};

fn transfer_started(status: &Result<JobStatus, PipeError>) -> bool {
    match status.as_ref() {
...
fn transfer_completed(status: &Result<JobStatus, PipeError>) -> bool {
    match status.as_ref() {
        Ok(job_status) => match job_status.state {
            BitsJobState::Error
...
fn status_to_nsresult(status: &Result<JobStatus, PipeError>) -> nsresult {
    match status.as_ref() {
        Ok(job_status) => match job_status.state {
...
impl MonitorRunnable {
    ...
    pub fn try_run(&self) -> Result<(), String> {
        if is_main_thread() {
            ...
            let tb_observer = self.observer.swap(None).ok_or("Missing observer")?;
            let maybe_tb_context = self.context.swap(None);
            let tb_bits_interface = self
                .bits_interface
                .swap(None)
            ...
            let maybe_next_thread: Option<RefPtr<nsIThread>> = {
                ...
                let bits_interface = tb_bits_interface
                    .get_ref()
                ...
                if !self.request_started.load() && transfer_started(&status) {
                    ...
                    let observer_result =
                        unsafe { observer.OnStartRequest(bits_interface.coerce(), context) }
                            .to_result();
                ...
                if transfer_completed(&status) {
                    ...
                    unsafe {
                        observer.OnStopRequest(bits_interface.coerce(), context, job_status);
                    }
