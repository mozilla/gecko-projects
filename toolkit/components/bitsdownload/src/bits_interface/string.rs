...
pub fn Guid_from_nsCString(
    ...
    let guid_string = nsCString_to_String(value, error_action, error_stage)?;
    Guid::from_str(&guid_string).map_err(|comedy_error| {
        BitsTaskError::from_comedy(ErrorType::InvalidGuid, error_action, error_stage, comedy_error)
    })
}
