...
use std::fmt::Debug;
use xpcom::{interfaces::nsIBITSSetPriorityLowCallback, RefPtr, ThreadBoundRefPtr};
...
impl Task for SetPriorityLowTask {
    fn run(&self) {
        ...
        match result {
            ...
            Err(task_error) => {
                self.result
                    .store(Some(SetPriorityLowResult::failure_from(task_error)));
            }
        }
    }
    ...
    fn done(&self) -> Result<(), nsresult> {
        ...
        match maybe_callback {
            Ok(callback) => match self.result.swap(None) {
                ...
                Some(SetPriorityLowResult::BITSFailure(val)) => unsafe {
