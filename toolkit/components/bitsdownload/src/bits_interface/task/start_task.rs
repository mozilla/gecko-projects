...
use super::{client::with_bits_client, error::BITSTaskError, string::nsCString_to_OsString};
...
impl Task for StartDownloadTask {
    fn run(&self) {
        let result = with_bits_client(|client| -> Result<StartDownloadResult, BITSTaskError> {
            let result = client.start_job(
                nsCString_to_OsString(&self.download_url)?,
                nsCString_to_OsString(&self.save_rel_path)?,
                self.proxy,
                self.update_interval_ms,
            );
            match result? {
                Ok((start_success, monitor_client)) => Ok(StartDownloadResult::success_from(
                    start_success.guid,
                    monitor_client,
                )),
        ...
        match result {
            ...
            Err(task_error) => {
                self.result
                    .store(Some(StartDownloadResult::failure_from(task_error)));
            }
        }
    }
    ...
    fn done(&self) -> Result<(), nsresult> {
        ...
        let maybe_callback = maybe_tb_callback
            ...
            .map_err(String::from);
        let maybe_bits_interface = maybe_tb_interface
            .as_ref()
            .ok_or("Unexpected: No BITS interface")
            .and_then(|ti| {
                ti.get_ref()
                    .ok_or("Unexpected: BITS interface on wrong thread")
            })
            .map_err(String::from);
        let maybe_observer = maybe_tb_observer
            .as_ref()
        ...
        let maybe_context: Result<Option<&nsISupports>, String> = match maybe_tb_context.as_ref() {
            Some(tb_context) => tb_context
                .get_ref()
                .ok_or("Unexpected: Observer context on wrong thread")
                .map(Option::from)
                .map_err(String::from),
            None => Ok(None),
        };
        ...
        let callback_result: Result<nsresult, String> = match self.result.swap(None) {
            Some(StartDownloadResult::Success(guid, monitor_client)) => {
                let monitor_result = maybe_bits_interface.and_then(|bits_interface| {
                    ...
                    let monitor_result = bits_interface
                ...
                maybe_callback.map(|callback| match monitor_result {
