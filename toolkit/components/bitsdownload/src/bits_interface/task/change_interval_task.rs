...
use std::fmt::Debug;
use xpcom::{
    interfaces::{nsIBITSChangeMonitorIntervalCallback},
...
impl Task for ChangeMonitorIntervalTask {
    fn run(&self) {
        let result = with_bits_client(|client| -> Result<ChangeMonitorIntervalResult, BITSTaskError> {
            let result = client.set_update_interval(Guid_from_nsCString(&self.id)?,
        ...
        match result {
            ...
            Err(task_error) => {
                self.result.store(Some(ChangeMonitorIntervalResult::failure_from(task_error)));
            },
        }
    }
