...
use super::{
    ...
    dispatch_callback::{CallbackExpected, maybe_dispatch_request_via_callback},
...
impl<D, S> Task for InterfaceTask<D, S> {
    fn run(&self) {
        let result = with_maybe_new_bits_client(
            &self.client_init_data,
            self.action.into(),
            |client| (self.run_fn)(&self.task_data, client)
        );
...
impl StartDownloadTask {
    ...
    fn run_fn(
        ...
        Ok(StartDownloadSuccess {guid: success.guid, monitor_client})
    }
