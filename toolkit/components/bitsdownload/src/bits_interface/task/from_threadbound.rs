...
impl DataType {
    fn error_type(&self, error: GetThreadboundError) -> ErrorType {
        match self {
            ...
            DataType::BitsInterface => match error {
                GetThreadboundError::Missing => ErrorType::MissingBitsInterface,
                GetThreadboundError::WrongThread => ErrorType::BitsInterfaceOnWrongThread,
            },
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            DataType::BitsInterface => "BITS Interface",
...
pub fn get_from_threadbound<T>(
    threadbound: &ThreadBoundRefPtr<T>,
    data_type: DataType,
    action: Action,
) -> Option<&T>
where
    ...
    T: RefCounted + 'static,
{
    maybe_threadbound
        .as_ref()
        .and_then(|threadbound| get_from_threadbound(threadbound, data_type, action))
}

pub fn expect_from_threadbound<T>(
    threadbound: &ThreadBoundRefPtr<T>,
    data_type: DataType,
    action: Action,
    ...
    T: RefCounted + 'static,
{
    match threadbound.get_ref() {
        ...
        None => Err(BitsTaskError::new(
            ...
            action,
            // Retreiving data from threadbounds all happens on the main thread.
            // No data is ever bound to the command thread so there would be no
    ...
    T: RefCounted + 'static,
{
    match maybe_threadbound.as_ref() {
        Some(threadbound) => expect_from_threadbound(threadbound, data_type, action),
        None => Err(BitsTaskError::new(
