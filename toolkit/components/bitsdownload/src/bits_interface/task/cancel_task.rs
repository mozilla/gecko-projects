...
use std::fmt::Debug;
use xpcom::{interfaces::nsIBITSCancelDownloadCallback, RefPtr, ThreadBoundRefPtr};
...
impl Task for CancelDownloadTask {
    fn run(&self) {
        ...
        match result {
            ...
            Err(task_error) => {
                self.result
                    .store(Some(CancelDownloadResult::failure_from(task_error)));
            }
        }
    }
    ...
    fn done(&self) -> Result<(), nsresult> {
        ...
        match self.result.swap(None) {
            Some(CancelDownloadResult::Success) => match maybe_bits_interface {
                Ok(bits_interface) => {
