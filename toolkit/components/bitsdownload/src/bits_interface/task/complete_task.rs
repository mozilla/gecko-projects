...
use std::fmt::Debug;
use xpcom::{interfaces::nsIBITSCompleteDownloadCallback, RefPtr, ThreadBoundRefPtr};
...
impl Task for CompleteDownloadTask {
    fn run(&self) {
        ...
        match result {
            ...
            Err(task_error) => {
                self.result
                    .store(Some(CompleteDownloadResult::failure_from(task_error)));
            }
        }
    }
    ...
    fn done(&self) -> Result<(), nsresult> {
        ...
        let callback_result: Result<nsresult, String> = match self.result.swap(None) {
            Some(CompleteDownloadResult::Success) => match maybe_bits_interface {
                Ok(bits_interface) => {
