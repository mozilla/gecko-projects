...
impl BitsRequest {
    ...
    /// BitsInterface's command thread, destroying the BitsClient.
    ...
    xpcom_method!(
        get_bits_transfer_result_nsIBitsRequest => GetBitsTransferResult() -> i32
    );
    #[allow(non_snake_case)]
    fn get_bits_transfer_result_nsIBitsRequest(&self) -> Result<i32, nsresult> {
        let error_type = match self.get_transfer_result() {
            None => nsIBits::ERROR_TYPE_SUCCESS as i32,
            Some(error_type) => error_type.bits_code(),
        };
        Ok(error_type)
    }
    pub fn get_transfer_result(&self) -> Option<ErrorType> {
        self.download_status_error_type.get()
    }
