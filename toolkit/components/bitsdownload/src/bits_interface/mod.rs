...
use xpcom::{
    ...
    interfaces::{
        ...
        nsIBITSCompleteDownloadCallback, nsIBITSInitCallback,
        nsIBITSMonitorDownloadCallback, nsIBITSResumeDownloadCallback,
        nsIBITSSetPriorityHighCallback, nsIBITSSetPriorityLowCallback,
        nsIBITSStartDownloadCallback, nsIBITSSuspendDownloadCallback,
        nsILoadGroup, nsIRequest, nsIRequestObserver, nsISupports, nsIThread,
        nsLoadFlags, nsProxyUsage,
    },
...
impl BITSInterface {
    ...
    fn start_monitor_thread(
        ...
        if monitor_thread.is_some() {
            ...
            return Err(BITSTaskError::with_message("Monitor thread already running"));
        }
        let monitor_runnable = MonitorRunnable::new(RefPtr::new(self),
                                                    id,
        ...
        let monitor_thread = match create_thread("BITSMonitor") {
            Ok(thread) => thread,
            Err(rv) => {
                return Err(BITSTaskError::with_nsresult("Unable to create monitor thread", rv));
            },
        };
        match monitor_runnable.dispatch(monitor_thread.clone()) {
            ...
            Err(rv) => {
                ...
                Err(BITSTaskError::with_nsresult("Unable to dispatch monitor runnable", rv))
            },
        }
    }
    ...
    fn shutdown_threads(&self) -> Result<(), nsresult> {
        ...
        if let Some(command_thread) = self.command_thread.take() {
            match unsafe { command_thread.AsyncShutdown() }.to_result() {
                Ok(()) => { self.command_thread.take(); },
                Err(rv) => { result = Err(rv); },
            }
        }
    ...
    unsafe fn StartDownload(
        ...
        ensure_param!(proxy);              // Neither of these should be needed, but just
        ensure_param!(update_interval_ms); // to be consistent with the xpcom_method macro
        ...
        match self.start_download(download_url,
                                  save_rel_path,
                                  proxy,
                                  update_interval_ms,
                                  observer,
    ...
    unsafe fn MonitorDownload(
        ...
        match self.monitor_download(id,
                                    update_interval_ms,
                                    observer,
                                    context,
                                    callback) {
    ...
    fn suspend_download(
        ...
        self.ensure_initialized()?;
        let task: Box<SuspendDownloadTask> = Box::new(SuspendDownloadTask::new(
            id,
            callback,
        ));
    ...
    fn resume_download(
        ...
        self.ensure_initialized()?;
        let task: Box<ResumeDownloadTask> = Box::new(ResumeDownloadTask::new(
            id,
            callback,
        ));
