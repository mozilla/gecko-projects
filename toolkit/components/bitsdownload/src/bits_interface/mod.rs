...
pub struct InitBitsService {
    ...
    // starts and decremented when the action ends and returns the result to
    // the callback via the callback. This prevents the command thread from
    // being shut down while a job is being created.
