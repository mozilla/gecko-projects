...
use self::{
    action::{Action, Action::{MonitorDownload, StartDownload}},
    error::{
        ErrorStage::Pretask,
        ErrorType::{
...
use nserror::{
    nsresult, NS_ERROR_ALREADY_INITIALIZED, NS_ERROR_NO_AGGREGATION, NS_OK,
};
...
impl BitsInterface {
    ...
    fn dispatch_runnable_to_command_thread(
        ...
        let command_thread = self.get_command_thread().map_err(|rv| {
            BitsTaskError::from_nsresult(FailedToStartThread, action, Pretask, rv)
        })?;
    ...
    fn start_download(
        ...
        let client_init_data = self.get_client_init().ok_or_else(|| {
            BitsTaskError::new(NotInitialized, StartDownload, Pretask)
        })?;
        if update_interval_ms >= client_init_data.monitor_timeout_ms {
            return Err(BitsTaskError::new(InvalidArgument, StartDownload, Pretask));
        }
        let proxy = match proxy as i64 {
            ...
            _ => return Err(BitsTaskError::new(InvalidArgument, StartDownload, Pretask)),
        };
    ...
    fn monitor_download(
        ...
        let client_init_data = self.get_client_init().ok_or_else(|| {
            BitsTaskError::new(NotInitialized, MonitorDownload, Pretask)
        })?;
        if update_interval_ms >= client_init_data.monitor_timeout_ms {
            return Err(BitsTaskError::new(InvalidArgument, MonitorDownload, Pretask));
        }
        let guid = Guid_from_nsCString(&nsCString::from(id), MonitorDownload, Pretask)?;
