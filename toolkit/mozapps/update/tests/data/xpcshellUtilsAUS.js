...
/**
 * Helper function to check whether the permissions on a file are correct.
 * This function should only be called on Windows platforms.
 *
 * @param    aPath
 *           The path to the file to check.
 * @return   true if the permissions are correct, false if they are not, or on
 *           failure.
 */
function permissionsOk(aPath) {
  if (AppConstants.platform != "win") {
    do_throw("Windows only function called by a different platform!");
  }
  let args = ["check-perms-correct", aPath];
  let exitValue = runTestHelperSync(args);
  debugDump(
    "Checking permissions on file. " +
      FILE_HELPER_BIN +
      " returned " +
      exitValue +
      " for file " +
...
/**
 * Helper function to set incorrect permissions on a file. The function will
 * throw an exception if the permissions could not be set.
 * This function should only be called on Windows platforms.
 *
 * @param    aPath
 *           The path to the file to change permissions on.
 */
function setBadPermissions(aPath) {
  if (AppConstants.platform != "win") {
    do_throw("Windows only function called by a different platform!");
  }
  let args = ["set-bad-perms", aPath];
  let exitValue = runTestHelperSync(args);
  if (exitValue != 0) {
    do_throw(
...
/**
 * Helper function to create a directory junction. The function will throw an
 * exception if the junction could not be created.
 * This function should only be called on Windows platforms.
 *
 * @param    aLinkPath
 *           The path where the directory junction should be created. There
 *           should either be no file/directory at this path, or there should
 *           be an empty directory.
 * @param    aTargetPath
 *           The path where the directory junction should point to. This must be
 *           a directory
 */
function makeDirJunction(aLinkPath, aTargetPath) {
  if (AppConstants.platform != "win") {
    do_throw("Windows only function called by a different platform!");
  }
  // The Test Helper expects the link destination to already be an empty
  // directory, so create that first.
  let link = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsIFile);
  link.initWithPath(aLinkPath);
  try {
    link.create(Ci.nsIFile.DIRECTORY_TYPE, FileUtils.PERMS_DIRECTORY);
  } catch (e) {
    // If we fail to create the directory, it might already exist. Just wait
    // and see if we successfully create the directory junction and determine
    // success/failure from that alone.
  }

  let args = ["make-dir-junction", aLinkPath, aTargetPath];
  let exitValue = runTestHelperSync(args);
  if (exitValue != 0) {
    do_throw(
      "Unable to make directory junction. " +
        FILE_HELPER_BIN +
        " returned " +
        exitValue +
        " for link " +
...
/**
 * Helper function to create a hard link. The function will throw an exception
 * if the hard link could not be created.
 * This function should only be called on Windows platforms.
 *
 * @param    aLinkPath
 *           The path to create the hard link at.
 * @param    aTargetPath
 *           The path that the hard link should point to.
 */
function makeHardLink(aLinkPath, aTargetPath) {
  if (AppConstants.platform != "win") {
    do_throw("Windows only function called by a different platform!");
  }
  let args = ["make-hard-link", aLinkPath, aTargetPath];
  let exitValue = runTestHelperSync(args);
  if (exitValue != 0) {
    do_throw(
...
/**
 * Helper function to fix permissions on the update directory and its contents.
 * The function will throw if the permissions could not be fixed.
 * This function should only be called on Windows platforms.
 * This function should not be used in service tests as a mechanism to invoke
 * the maintenance service because nsIUpdateProcessor::fixUpdateDirectoryPerms
 * does not guarantee that the service will ever be invoked.
 *
 * @return   true on success, false on failure
 */
async function fixUpdateDirectoryPerms() {
  if (AppConstants.platform != "win") {
    do_throw("Windows only function called by a different platform!");
  }
  if (gIsServiceTest) {
    do_throw(
      "fixUpdateDirectoryPerms called from a service test, but " +
        "fixUpdateDirectoryPerms is unable to force service usage."
    );
  }
  let promisePermsFixed = new Promise((resolve, reject) => {
    let successObserver = (aSubject, aTopic, aData) => {
      Services.obs.removeObserver(successObserver, "update-perms-fixed");
      Services.obs.removeObserver(failureObserver, "update-error");
      resolve();
    };
    let failureObserver = (aSubject, aTopic, aData) => {
      if (aData != "bad-perms") {
        return;
      }
      Services.obs.removeObserver(successObserver, "update-perms-fixed");
      Services.obs.removeObserver(failureObserver, "update-error");
      reject("fixUpdateDirectoryPerms failed");
    };
    Services.obs.addObserver(successObserver, "update-perms-fixed");
    Services.obs.addObserver(failureObserver, "update-error");
  });

  logTestInfo("Attempting to fix permissions");
  Cc["@mozilla.org/updates/update-processor;1"]
    .createInstance(Ci.nsIUpdateProcessor)
    .fixUpdateDirectoryPerms(false);

  return promisePermsFixed;
}
