...
/**
 * Obtains the path of the Temp directory in the Windows directory.
 *
 * @param  destBuf
...
BOOL GetWindowsTempFilePath(LPWSTR destBuf) {
  ...
  if (!PathAppendSafe(destBuf, L"Temp")) {
    return FALSE;
  }
...
BOOL WriteStatusFailure(LPCWSTR patchDirPath, int errorCode) {
  WCHAR winTempDirPath[MAX_PATH];
  if (!GetWindowsTempFilePath(winTempDirPath)) {
    return FALSE;
  }

  WCHAR updateDirName[MAX_PATH];
