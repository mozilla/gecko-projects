...
static HRESULT GetFilename(SimpleAutoString& path,
...
class FileOrDirectory {
 private:
  ...
  bool Lock(const wchar_t* path) {
    ...
    if (isDir == Tristate::True) {
      ...
      mLockHandle.own(CreateFileW(lockPath.String(),
                                  0,
                                  0,
                                  nullptr,
                                  OPEN_ALWAYS,
      ...
      mLockHandle.own(CreateFileW(path,
 ...
 public:
   ...
   * IsLocked method.
   */
  FileOrDirectory(const SimpleAutoString& path, Locked shouldLock)
  : FileOrDirectory()
  {
   ...
   * IsLocked method.
   */
  void Reset(const SimpleAutoString& path, Locked shouldLock) {
    Unlock();
    mDirLockFilename.Truncate();
    if (shouldLock == Locked::Locked) {
    ...
    if (IsLocked() && IsDirectory() == Tristate::False) {
      ...
      handle = CreateFileW(path.String(),
                           0,
                           FILE_SHARE_READ,
                           nullptr,
                           OPEN_EXISTING,
  ...
  void Unlock() {
    mLockHandle.own(INVALID_HANDLE_VALUE);
  }

  bool IsLocked() const {
    return mLockHandle.get() != INVALID_HANDLE_VALUE;
  }
  ...
  Tristate PermsOk(const SimpleAutoString& path, const AutoPerms& perms) {
    ...
    DWORD drv = GetSecurityInfo(handle,
                  SE_FILE_OBJECT,
...
static HRESULT EnsureUpdateDirectoryPermissions(
                 const SimpleAutoString& basePath,
...
static HRESULT GetUpdateDirectory(const wchar_t* installPath,
  ...
  if (whichDir == WhichUpdateDir::CommonAppData) {
    if (updatePath.Length() > 0) {
      hrv =
          EnsureUpdateDirectoryPermissions(basePath, updatePath, true,
                                           permsToSet);
    } else {
      hrv =
          EnsureUpdateDirectoryPermissions(basePath, basePath, false,
                                           permsToSet);
    }
    if (FAILED(hrv)) {
      return hrv;
 ...
 * directory, that a malicious process does not sneak a hard link into that
 * directory (causing it to inherit the permissions set on the directory).
 *
...
static HRESULT EnsureUpdateDirectoryPermissions(
  ...
  Locked shouldLock = permsToSet == SetPermissionsOf::AllFilesAndDirs ?
  ...
  // The permissions checking won't create the base path. If it didn't exist,
  // or was a file/link (which should be removed by EnsureCorrectPermissions),
  // If this fails, it's no big deal. Presumably the directory already exists.
  // But if it succeeds, we should consider this entire function to have been a
  // success as the directory exists and it and its (lack of) contents have the
  // correct permissions.
  ...
  // Check that the directory exists before we exit. It would be bad if we
  // returned success but the update directory doesn't exist.
  if (SUCCEEDED(returnValue)) {
    baseDir.Reset(basePath, Locked::Unlocked);
...
static HRESULT MakeDir(const SimpleAutoString& path, const AutoPerms& perms) {
  ...
  FileOrDirectory dir(path, Locked::Unlocked);
 ...
 * Non-directory files and that conflict with this are moved or deleted.
 *
...
static HRESULT EnsureCorrectPermissions(SimpleAutoString& path,
  ...
  if (file.IsDirectory() != Tristate::True || file.IsLink() != Tristate::False) {
  ...
  for (dirent* entry = readdir(&directoryHandle); entry;
    ...
    FileOrDirectory child(childBuffer, Locked::Locked);
...
static HRESULT FixDirectoryPermissions(const SimpleAutoString& path,
  ...
  success = CreateDirectoryW(path.String(),
                  const_cast<LPSECURITY_ATTRIBUTES>(&perms.securityAttributes));
  if (!success) {
    return E_FAIL;
  }
  directory.Reset(path, Locked::Locked);
  ...
  FileOrDirectory tempDir(tempPath, Locked::Locked);
  ...
  for (dirent* entry = readdir(&directoryHandle); entry;
    ...
    moveTo.AssignSprintf(MAX_PATH + 1, L"%s\\%s", path.String(),
                         entry->d_name);
    if (moveTo.Length() == 0) {
      returnValue = FAILED(returnValue)
                        ? returnValue
...
static HRESULT GetFilename(SimpleAutoString& path,
  ...
  const wchar_t* separator = (separator1 > separator2) ? separator1
                                                       : separator2;
