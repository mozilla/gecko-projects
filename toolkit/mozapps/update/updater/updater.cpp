...
#ifdef XP_WIN
/**
 * Obtains the FILETIME for the secure update status file.
 *
 * @param  fileTime
 *         A FILETIME structure to store the result.
 * @return true if successful
 */
bool GetSecureStatusFileCreationTime(FILETIME fileTime) {
  ...
  HANDLE hStatusFile =
      CreateFileW(statusFilePath, GENERIC_READ, 0, nullptr, OPEN_EXISTING,
                  FILE_ATTRIBUTE_NORMAL, nullptr);
  if (hStatusFile == INVALID_HANDLE_VALUE) {
    return false;
  }

  if (!GetFileTime(hStatusFile, &fileTime, nullptr, nullptr)) {
...
static bool WriteToFile(const NS_tchar* aFilename, const char* aStatus) {
  NS_tchar filePath[MAXPATHLEN] = {NS_T('\0')};
#if defined(XP_WIN)
  if (sUseSecureOutputPath) {
    if (!GetSecureOutputFilePath(gPatchDirPath, L".status", filePath)) {
      return false;
    }
  } else {
    NS_tsnprintf(filePath, sizeof(filePath) / sizeof(filePath[0]),
                 NS_T("%s\\%s"), gPatchDirPath, aFilename);
  }
#else
  NS_tsnprintf(filePath, sizeof(filePath) / sizeof(filePath[0]), NS_T("%s/%s"),
  ...
  {
    AutoFile file(NS_tfopen(filePath, NS_T("wb+")));
    if (file == nullptr) {
      return false;
    }
...
#  endif
      ...
      if (useService) {
        // Get the secure status file's timestamp before trying to update so it
        // is possible to determine if the updater or the maintenance service
        // has created a new one.
        FILETIME ftCreateBefore;
        bool checkStatusFileTime =
            GetSecureStatusFileCreationTime(ftCreateBefore);
        ...
        if (useService) {
          ...
          if (lastState != SERVICE_STOPPED) {
            ...
            // Copy the secure output files if the secure status file's
            // timestamp has changed.
            sCopyOutputFiles = true;
            FILETIME ftCreateAfter;
            if (checkStatusFileTime &&
                GetSecureStatusFileCreationTime(ftCreateAfter) &&
                CompareFileTime(&ftCreateBefore, &ftCreateAfter) == 0) {
              LOG(
                  ("The secure update status file's timestamp hasn't changed "
                   "after launching the updater using the service"));
      ...
      if (!useService && !noServiceFallback &&
        ...
        // Get the secure status file's timestamp before trying to update so it
        // is possible to determine if the updater has created a new one.
        FILETIME ftCreateBefore;
        bool checkStatusFileTime =
            GetSecureStatusFileCreationTime(ftCreateBefore);
        ...
        if (result) {
          ...
          // Copy the secure output files if the secure status file's timestamp
          // has changed.
          sCopyOutputFiles = true;
          FILETIME ftCreateAfter;
          if (checkStatusFileTime &&
            ...
            LOG(
                ("The secure update status file's timestamp hasn't changed "
                 "after launching the updater using runas"));
            sCopyOutputFiles = false;
          }
        } else {
