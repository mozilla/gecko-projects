...
const {BitsConstants, BitsError, BitsUnknownError} =
...
var AUSTLMY = {
  ...
  /**
   * Submit a telemetry ping for an error downloading an update via BITS.
  ...
  pingBitsError: function UT_pingBitsError(aIsComplete, aError) {
    ...
    if (isNaN(type)) {
      ...
      type = BitsConstants.ERROR_TYPE_UNKNOWN;
    } else if (type == BitsConstants.ERROR_TYPE_SUCCESS) {
    ...
    if (aError.codeType == BitsConstants.ERROR_CODE_TYPE_HRESULT) {
      let scalarKey;
      if (aIsComplete) {
  ...
  /**
   * Submit a telemetry ping for success downloading an update via BITS.
   *
   * @param aIsComplete
   *        If true the histogram is for a patch type complete, if false the
   *        histogram is for a patch type partial. This will determine the
  ...
  pingBitsSuccess: function UT_pingBitsSuccess(aIsComplete) {
    ...
    this._pingBitsResult(aIsComplete, BitsConstants.ERROR_TYPE_SUCCESS);
  },
