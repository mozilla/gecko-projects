...
Downloader.prototype = {
  ...
  _canUseBits: function Downloader__canUseBits(patch) {
    if (AppConstants.platform != "win") {
      LOG("Downloader:_canUseBits Not using BITS because this is not Windows");
      return false;
    }
    if (!Services.prefs.getBoolPref(PREF_APP_UPDATE_BITS_ENABLED, true)) {
      LOG("Downloader:_canUseBits Not using BITS. Disabled by pref.");
      return false;
    }
    ...
    if (Services.prefs.getIntPref(PREF_NETWORK_PROXY_TYPE, defaultProxy) !=
        defaultProxy) {
      LOG("Downloader:_canUseBits Not using BITS because of proxy usage");
      return false;
    }
