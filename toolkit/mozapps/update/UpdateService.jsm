...
UpdatePrompt.prototype = {
  ...
  showUpdateHistory: function UP_showUpdateHistory(parent) {
    this._showUI(parent, "chrome://mozapps/content/update/history.xul",
                 "modal,dialog=yes");
  },
...
//    if (AppConstants.platform != "macosx") {
//      LOG("UpdatePrompt:showUpdateElevationRequired - called on an " +
//          "unsupported platform, returning early.");
//      return;
//    }
    let um = Cc["@mozilla.org/updates/update-manager;1"].
             getService(Ci.nsIUpdateManager);
    if (!um.activeUpdate) {
      LOG("UpdatePrompt:showUpdateElevationRequired - called without an " +
          "active update, returning early.");
      return;
    }
    this._showUI(null, "chrome://mozapps/content/update/updateElevation.xul",
                 "Update:Elevation");
  },
   ...
   *          The features in addition to the default features specified when
   *          calling openWindow.
   */
  _showUI: function UP__showUI(parent, uri, features, name = "") {
    let openFeatures = "chrome,centerscreen,dialog=no,resizable=no,titlebar,toolbar=no";
    if (features) {
      openFeatures += "," + features;
    }
    Services.ww.openWindow(parent, uri, name, openFeatures, null);
  },
