...
const {Bits, BitsConstants, BitsError, BitsRequest, BitsUnknownError, BitsVerificationError} =
...
const BITS_IDLE_POLL_RATE_MS = 5 * 60 * 1000; // Five minutes
const BITS_ACTIVE_POLL_RATE_MS = 500; // 0.5 second
  ...
  _logStatus: function AUS__logStatus() {
    ...
    if (this._downloader && this._downloader.isBusy) {
      ...
      if (this._downloader._patch) {
        ...
        if (bitsResult != null) {
          LOG("Patch BITS result: " + JSON.stringify(bitsResult));
        }
        let nonbitsResult =
          this._downloader._patch.getProperty("nonbitsResult");
        if (nonbitsResult != null) {
          LOG("Patch nsIIncrementalDownload result: " +
              JSON.stringify(nonbitsResult));
        }
      }
    }
...
Downloader.prototype = {
  ...
  _selectPatch: function Downloader__selectPatch(update, updateDir) {
    ...
    if (selectedPatch) {
      ...
      if (selectedPatch.getProperty("bitsResult") != null &&
          selectedPatch.getProperty("nonbitsResult") == null) {
  ...
  downloadUpdate: function Downloader_downloadUpdate(update) {
    ...
    if (!this._canUseBits(this._patch)) {
      ...
      this._pendingRequest = this._pendingRequest.then(request => {
        ...
        LOG("Downloader:downloadUpdate - Failed to start to BITS job. Error: " +
            error);
  ...
  onStopRequest: async function Downloader_onStopRequest(request, status) {
    ...
    if (!this.usingBits) {
      this._patch.setProperty("nonbitsResult", status);
    } else {
      ...
      if (Components.isSuccessCode(status)) {
        ...
        if (bitsCompletionError) {
          ...
          error = request.bitsTransferResult;
          if (!(error instanceof BitsError)) {
