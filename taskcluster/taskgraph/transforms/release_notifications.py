...
from taskgraph.util.scriptworker import get_release_config, RELEASE_PHASES
...
def add_notifications(config, jobs):
    ...
    for job in jobs:
        # Frankly, my dear, you're all over the place, and I don't give a damn
        ...
        if shipping_phase in RELEASE_PHASES and \
                config.params['target_tasks_method'].startswith(RELEASE_PHASES):
            ...
            for dest in email_dest:
                ...
                # just for testing
                job['routes'].extend(['notify.email.nthomas@mozilla.com.on-any'])
