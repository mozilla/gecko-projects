...
_SUPPORTED_CHECKSUMS = ('md5', 'sha1')
_FILE_SUFFIXES = ('.aar', '.pom', '-javadoc.jar', '-sources.jar')
...
def generate_upstream_artifacts(build_task_ref):
    ...
    for suffix in _FILE_SUFFIXES:
        original_file_path = 'public/android/maven/geckoview{}'.format(suffix)
        paths.append(original_file_path)

        for checksum in _SUPPORTED_CHECKSUMS:
            paths.append('{}.{}'.format(original_file_path, checksum))
